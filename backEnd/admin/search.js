...
async function courseArrangedSearch(res, val) {//如果是课程列表查询 走这个函数
    let model = Models.admin.CourseArrangedModel;
    let results = [];
    let promiseAll = [];
    try {
        //根据输入内容在学生表中模糊查找对应的学生姓名
        let Students = await Models.admin.StudentModel.find({name: {$regex: val, $options: 'i'}});
        //根据输入内容在教师表中模糊查找对应的教师姓名
        let Teachers = await Models.admin.TeacherModel.find({name: {$regex: val, $options: 'i'}});
        let results = [];
        ...
        if (promiseAll.length) {
            Promise.all(promiseAll).then(async (Courses) => {
                for (let i = 0; i < Courses.length; i++) {
                    if (Courses[i] !== null) {
                            let StudentName = await Models.admin.StudentModel.findOne({sno: Courses[i].sno}, 'name');
                            let TeacherName = await Models.admin.TeacherModel.findOne({workNumber: Courses[i].workNumber}, 'name');
                            let Course = await Models.admin.CourseModel.findOne({courseNo: Courses[i].courseNo}, 'course grade');
